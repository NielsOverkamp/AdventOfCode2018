...
event_re = re.compile(r"\[1518-(\d\d)-(\d\d) (\d\d):(\d\d)\] (?:(wakes up)|(falls asleep)|Guard #(\d+) begins shift)")

current_guard = None
previous_minute = None

events = sorted(event_re.findall(inp))

asleep = defaultdict(Counter)

for _, _, _, minute, wake, sleep, shift in events:
    minute = int(minute)
    if wake != '':
        for i_minute in range(previous_minute, minute):
            asleep[current_guard][i_minute] += 1
    elif sleep != '':
...
def reduce_asleep(item0, item1):
    _, stats0 = item0
    _, stats1 = item1
    max0 = max(stats0.items(), key=lambda a: a[1])
    max1 = max(stats1.items(), key=lambda a: a[1])
    if max0 > max1:
        return item0
    else:
        return item1
...
guard, stats = reduce(reduce_asleep, asleep.items())

print(int(guard) * max(stats.items(), key=lambda a: a[1])[0])
