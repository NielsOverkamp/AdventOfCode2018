...
inp = """position=< 20168,  40187> velocity=<-2, -4>
...
inp_reg = re.compile(r"(-?\d+)[^-\d]+(-?\d+)[^-\d]+(-?\d+)[^-\d]+(-?\d+)[^-\d]+")

inpr = list(map(lambda x: list(map(int, x)), inp_reg.findall(inp)))
...
for point_info in inpr:
    point = Point(*point_info)
    point.step(8000)
    points.append(point)
...
while True:
    ...
    for point in points:
        ...
        point.single_step()
    if (prev_min_x > min_x) or (prev_min_y > min_y) or (prev_max_x < max_x) or (prev_max_y < max_y):
        print(s)
        break
    prev_min_x = min_x
    prev_max_x = max_x
...
point_dict = defaultdict(lambda: False)
...
for point in points:
    ...
    point_dict[(point.x, point.y)] = True
    min_x = min(min_x, point.x)
    max_x = max(max_x, point.x)
    min_y = min(min_y, point.y)
    max_y = max(max_y, point.y)
...
while True:
    for y in range(min_y, max_y + 1):
        ...
        for x in range(min_x, max_x + 1):
            s.append("#" if point_dict[(x, y)] else ".")
        print(''.join(s))
